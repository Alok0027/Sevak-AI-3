...
5. Provision an independent scheduler, sharing the API's database, encryption key and provider configuration. Run `python -m scripts.process_notifications` from `backend` once per minute. `render.yaml` now defines this as a `sevakai-escalation-tick` cron service (same Dockerfile, same env var shape as the web service) -- it still has to be enabled from the Render dashboard, and Render Cron Jobs need at least the "starter" plan, which the free web/DB services in this blueprint do not require. This command sends messages when real providers are configured. Do not run it against real data as a smoke test. No scheduler is running until that plan is enabled.
