...
class Visit(Base):
    ...
    Agent 2's output (FR-03.1). transcript/structured_json are encrypted at
    rest (NFR-SC1) -- the raw voice transcript and its extracted fields are
    the most sensitive clinical content in the whole schema, and neither is
    ever filtered/grouped-by in SQL (only checked with .isnot(None)), so
    encryption is transparent to every existing query."""
    ...
    transcript: Mapped[str] = mapped_column(EncryptedString, nullable=True)
    structured_json: Mapped[str] = mapped_column(EncryptedString, nullable=True)  # JSON string: Agent 1 output
